...
function newLevel(){
	...
	var response=android.net.http.AndroidHttpClient.newInstance("Online()").execute(new org.apache.http.client.methods.HttpGet("https://raw.githubusercontent.com/wilco375/Minecraft-PE-Mod-Scripts/master/VeinMiner_Update_Checker_V1.2.txt")).getEntity().writeTo(out); 
	out.close(); 
...
function modTick(){
		...
		try{
			...
			if(Level.getTile(blocksVar[0][0],blocksVar[0][1],blocksVar[0][2])==blockIdVar && Level.getData(blocksVar[0][0],blocksVar[0][1],blocksVar[0][2])==blockDataVar){
				if(blockIdVar == itemToDropId){
					if(mcVersion<12) Level.destroyBlock(blocksVar[0][0],blocksVar[0][1],blocksVar[0][2],true);
					else{
					...
					if(mcVersion<12) Level.destroyBlock(blocksVar[0][0],blocksVar[0][1],blocksVar[0][2],false);
	...
	//Show button on < 0.12
	if(buttonShowed != 1 && carriedItem.in(257,278,285,258,286,279,275,271,274,270,256,269,273,277,284) && mcVersion < 12){
		buttonShowed = 1;
