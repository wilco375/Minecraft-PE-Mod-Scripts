...
var spellRadius = 10;
var spells = ["Fire","Air","Projectile","Explosion","Projectile Explosion","Instakill","Jump","Rain","Speed","Craft","Grow","Water","Lava"];
var spellsDesc = [
...
function newLevel(){
	...
	var response = android.net.http.AndroidHttpClient.newInstance("Online()").execute(new org.apache.http.client.methods.HttpGet("https://raw.githubusercontent.com/wilco375/Minecraft-PE-Mod-Scripts/master/SpellCraft_v1.0_update.txt")).getEntity().writeTo(out);
	out.close();
	...
	if(Player.getCarriedItem() == spellId){
		...
		switch(getCarriedItemName()){
			...
			case "Explosion Spell":
				...
				break;
			case "Rain Spell":
				castRainSpell(Entity.getX(victim),Entity.getY(victim),Entity.getZ(victim));
				break;
			default:
				castSpell();
		}
...
function handleSpellGUI(){
	if(!buttonShowed && Player.getCarriedItem() == spellId && getCarriedItemName().in("Projectile Spell","Projectile Explosion Spell")){
        buttonShowed = true;
    ...
    }else if(buttonShowed && (Player.getCarriedItem() != spellId || !getCarriedItemName().in("Projectile Spell","Projectile Explosion Spell"))){
        buttonShowed = false;
        ctx.runOnUiThread(new java.lang.Runnable({ run: function() {
            if(buttonWindow != null) {
                buttonWindow.dismiss();
	...
	if(Player.getCarriedItem() == spellId){
		if(!essenceShowed){
			...
			ctx.runOnUiThread(new java.lang.Runnable({ run: function() {
				try{
					...
					print("Essence GUI could not be displayed: " + e);
				}
			}}));
		...
		ctx.runOnUiThread(new java.lang.Runnable({ run: function() {
			try{
				...
				print("Essence GUI could not be dismissed: " + e);
			}
		}}));
	}
}
