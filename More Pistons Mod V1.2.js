...
function newLevel(){     
    ...
    var response = android.net.http.AndroidHttpClient.newInstance("Online()").execute(new org.apache.http.client.methods.HttpGet("https://raw.githubusercontent.com/wilco375/Minecraft-PE-Mod-Scripts/master/More_pistons_mod_1.2.js")).getEntity().writeTo(out); 
    out.close(); 
...
function destroyBlock(x,y,z,shouldDropItem){
	...
	if(blockId.in(pistonId,stickyPistonId,doublePistonId,doubleStickyPistonId,triplePistonId,tripleStickyPistonId,quadruplePistonId,quadrupleStickyPistonId,ultraStickyPistonId)){
		preventDefault();
		if(data < 6){
			preventDefault();
	...
	}else if(blockId == fakeRedstoneBlockId){
		preventDefault();
		if(Level.getGameMode() == 0) Level.dropItem(x,y,z,0.5,152,1,0);
		setTile(x,y,z,0);
	}else if(blockId == pistonExtensionId) preventDefault();
}
	...
	if(active && blockId.in(pistonId,stickyPistonId,doublePistonId,doubleStickyPistonId,triplePistonId,tripleStickyPistonId,quadruplePistonId,quadrupleStickyPistonId,ultraStickyPistonId) && blockDamage < 6 && getTile(x+dirs[blockDamage][0],y+dirs[blockDamage][1],z+dirs[blockDamage][2])!=152){
		...
		for(k = 1;k<=extendFor;k++){
			...
			for (var i = 1; i <= maxBlocksToPush; i++) { 
				if (getTile(x + (i*dir[0]), y + (i*dir[1]), z + (i*dir[2])) == 0 || shouldDrop(getTile(x + (i*dir[0]), y + (i*dir[1]), z + (i*dir[2])))) { 
			...
			if (pistonCount != -1) { 
				...
				for (var i = pistonCount - 1; i >= 0; i--) { 
					...
					if(blockId == fakeRedstoneBlockId) blockToMoveId = 152;
					var blockToDrop = dropBlock(blockToMoveId);
					if(shouldDrop(blockToMoveId)){
						setTile(x + ((i+1)*dir[0]), y + ((i+1)*dir[1]), z + ((i+1)*dir[2]),0);
						Level.dropItem(x + ((i+1)*dir[0]), y + ((i+1)*dir[1]), z + ((i+1)*dir[2]),0.5,blockToDrop,0);
					}else setTile(x + ((i+1)*dir[0]), y + ((i+1)*dir[1]), z + ((i+1)*dir[2]), blockToMoveId, blockToMoveData); 
				} 
				if(k == 1){
					//Play piston sound
...
function dropBlock(blockId){
	if(blockId == 26) return 355;
	else if(blockId == 140) return 390;
	else if(blockId == 55) return 331;
	else if(blockId == 75)	return 76;
	else if(blockId == 83) return 338;
	else if(blockId == 103) return 360;
	else if(blockId == 30) return 287
}

function shouldDrop(blockId){
	if(blockId.in(26,37,38,81,64,71,140,70,72,86,39,40,55,75,76,83,50,147,148,103,30)) return true;
	else return false;
}
