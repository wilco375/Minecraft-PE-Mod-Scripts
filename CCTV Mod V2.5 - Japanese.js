...
function selectLevelHook(){
	if(ModPE.readData("cameras"+Level.getWorldDir()) != null && ModPE.readData("cameras"+Level.getWorldDir()) != ""){
		...
		ctx.runOnUiThread(new java.lang.Runnable(){
			run: function(){
				android.widget.Toast.makeText(ctx, "[CCTV Mod] "+cameras.length+" 個のカメラが読み込まれました", 0).show();
			}
		});
	}
}
	...
	ctx.runOnUiThread(new java.lang.Runnable(){
		run: function(){
			try{
				...
				dialogBuilder.setTitle("名前を入力してください");
				...
				dialogBuilder.setPositiveButton("Ok", new android.content.DialogInterface.OnClickListener(){
					onClick: function(){ 
						try{
							...
							if(name != ""){
								showClientMessage("このカメラの名前を設定しました! " + name);
								if(cameras == null){
									cameras = []
									camerasX = []
									camerasY = []
...
function procCmd(command){
	...
	if(cmd[0] == "cctv"){
		...
		//Show a camera
			...
			if(goToCam == null){
				showClientMessage("このカメラは存在しません。 »/cctv cameras« と打ってカメラの一覧を確認します。")
			}
		}
		//Leave the current camera
			...
			else{showClientMessage("あなたがカメラを通して見ていません")}
		}
		//Remove a camera from the list
			...
			for(i = 0;i < cameras.length;i++){
				if(cameras[i] == camToRemove){
					...
					camerasZ.splice(i,1)
					removed = 1
					showClientMessage("カメラの削除に成功しました")
				}
			}
			if(removed != 1) showClientMessage("この名前のカメラが見つかりません。")
			removed = 0
		}
		//List all the commands
		else if(cmd[1] == "list"){
			showClientMessage("»/cctv cameras«, すべてのカメラの名前")
			showClientMessage("»/cctv show <camera name>«, そのカメラの映像を見ます")
			showClientMessage("»/cctv leave«, カメラを見るのをやめて元の位置に戻ります")
			showClientMessage("»/cctv remove <camera name>«,  このコマンドでカメラをリストから消すことができます")
		}
	}
}
	...
	ctx.runOnUiThread(new java.lang.Runnable(){
		run: function(){
			try{		
				...
				dialogBuilder.setTitle("カメラのリスト");
...
function showCameraFromList(goToCam){
	...
	if(getTile(cameraX,cameraY,cameraZ) == cameraId){
		...
		if(!camera){
			showClientMessage("»/cctv leave« と打つかモニターにある[カメラから去る]の欄を押すとカメラから離れることができます")
			camera = true
		}else{
		...
		showClientMessage("この位置にあるカメラは、もはやカメラではありません!");
