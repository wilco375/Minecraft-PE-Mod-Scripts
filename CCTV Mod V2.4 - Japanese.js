//CCTV Mod V2.4 (Compatible with 0.14)
//by wilco375
//日本語化 takutaku716
...
Block.defineBlock(cameraId,"カメラ",["piston_inner",0],18,false,0)
...
ModPE.setItem(monitorId,"item_frame",0,"モニター")
...
function selectLevelHook(){
	if(ModPE.readData("cameras"+Level.getWorldDir()) != null && ModPE.readData("cameras"+Level.getWorldDir()) != ""){
		...
		print(cameras.length+" 個のカメラが読み込まれました")
	}
}

function newLevel(){
	var out = new java.io.ByteArrayOutputStream();
	var response = android.net.http.AndroidHttpClient.newInstance("Online()").execute(new org.apache.http.client.methods.HttpGet("https://raw.githubusercontent.com/wilco375/Minecraft-PE-Mod-Scripts/master/CCTV%202.4%20Update%20Checker.txt")).getEntity().writeTo(out);
	out.close();
...
function leaveGame(){
	...
	print(cameras.length + " 個のカメラが保存されました")
	cameras = null
	camerasX = null
	camerasY = null
	camerasZ = null
				...
				button.setOnClickListener(new android.view.View.OnClickListener(){
					onClick: function(){ 
						try{
							...
							if(name != ""){
								dialog.dismiss();
								clientMessage("このカメラの名前を設定しました! " + name);
								if(cameras == null){
									cameras = []
									camerasX = []
									camerasY = []
...
function procCmd(command){
	...
	if(cmd[0] == "cctv"){
		...
		//Show a camera
			...
			if(goToCam == null){
				clientMessage("このカメラは存在しません。 »/cctv cameras« と打ってカメラの一覧を確認します。")
			}
		}
		//Leave the current camera
			...
			else{clientMessage("あなたがカメラを通して見ていません")}
		}
		//Remove a camera from the list
			...
			for(i = 0;i < cameras.length;i++){
				if(cameras[i] == cmd[2]){
					...
					camerasZ.splice(i,1)
					removed = 1
					clientMessage("カメラの削除に成功しました")
				}
			}
			if(removed != 1) clientMessage("この名前のカメラが見つかりません。")
			removed = 0
		}
		//List all the commands
		else if(cmd[1] == "list"){
			clientMessage("»/cctv cameras«, すべてのカメラの名前")
			clientMessage("»/cctv show <camera name>«, そのカメラの映像を見ます")
			clientMessage("»/cctv leave«, カメラを見るのをやめて元の位置に戻ります")
			clientMessage("»/cctv remove <camera name>«,  このコマンドでカメラをリストから消すことができます")
		}
	}
}
...
function showCameraFromList(goToCam){
	if(camera != 1){
		...
		if(getTile(cameraX,cameraY,cameraZ) == cameraId){
			...
			clientMessage("»/cctv leave« と打つかモニターにある[カメラから去る]の欄を押すとカメラから離れることができます")
			camera = 1
		}
		else{
			clientMessage("この位置にあるカメラは、もはやカメラではありません!")
			}
	}
	else{
		prevCameraX = cameraX
		...
		else clientMessage("この位置にあるカメラは、もはやカメラではありません!") 
	}
}